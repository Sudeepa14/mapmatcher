...
def mapMatch(track, segments, decayconstantNet = 30, decayConstantEu = 10, maxDist = 50, addfullpath = True):
    ...
    # point1 = arcpy.Point(51.467057004358743, 5.474460367112933)
    # ptGeometry1 = arcpy.PointGeometry(point1)

    # point2 = arcpy.Point(51.465963331817377, 5.473848990808071)
    # ptGeometry2 = arcpy.PointGeometry(point2)
    # points.append(ptGeometry1)
    # points.append(ptGeometry2)
...
def getpointMatches(points, path):
    qr =  '"osmid" IN ' +str(tuple(path))
    arcpy.SelectLayerByAttribute_management('segments_lyr',"NEW_SELECTION", qr)
    opta = []
    for point in points:
        sdist = 100000
        candidate = ''
        cursor = arcpy.da.SearchCursor('segments_lyr', ["osmid", "SHAPE@"])
        for row in cursor:
...
def getNDProbability(dist,s2,decayconstant = 30):
    """
    ...
    dist = float(dist)
    try:
        # road=[1513646, 1513645, 1513627, 1513642, 1513644, 1513648, 1513647, 1513640, 1513649, 1513651, 1513681, 1513623, 1513624, 1514829, 1514822, 1514805, 1514766, 1514808, 1520336]
        # if(s2 in road):
        #     print("********************************************************************\n")
        #     dist=dist/10
...
##        else:
    ...
    return (getNDProbability(dist,s2,decayconstantNet),subpath,s2_point)
...
def getTrackPoints(track, segments):
    ...
    if arcpy.Exists(track):
        ...
        # sdd
        # p1=Point(103.8519008,1.3391704)
        # p2=Point(103.8489586,1.337456)
        # <PointGeometry object at 0x1a0ca096470[0x1a0c9e51da0]
...
def getSegmentInfo(segments):
    ...
    if arcpy.Exists(segments):
        cursor = arcpy.da.SearchCursor(segments, ["osmid", "SHAPE@"])
        endpoints = {}
        segmentlengths = {}
        for row in cursor:
...
##    #Test using the shipped data example
    ...
    # nx.write_shp(G,'sudeepa.shp') # doctest +SKIP
    

    opt = mapMatch('testTrack.shp', 'testSegments.shp', 25, 10, 50)
    # opt = mapMatch('shapename.shp', 'edges.shp', 25, 10, 50)
    
    #outputs testTrack_path.shp
    print("this is the final")
    print(opt)    
    exportPath(opt, 'testTrack1.shp')
